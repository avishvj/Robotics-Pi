...
def calculateLikelihoodForAllParticles(particles, z):
    for p in particles:
        ...
        differenceBetweenActualDistanceAndExpected(p[0], p[1], p[2], z)
...
def differenceBetweenActualDistanceAndExpected(x, y, theta, z, particles):
    error = getClosestWallToParticle(x, y, theta) - z
    if (error > 15) :
        particles.append[(x + z * math.cos(theta), y + z * math.sin(theta))])
